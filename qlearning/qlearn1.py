...
class field:
    ...
    def toPrint(self):
        f = False
        try:
            with open('toPrint.txt','r', encoding='utf-8') as f:
                n = f.read()
                if n == '1':
                    return True
                else:
    ...
    def play(self):
        ...
        while not finish:
            ...
            if self.features[self.catcherstates[0]][self.actions[0]]!=-10000000:
                self.features[self.catcherstates[0]][self.actions[0]] = self.features[self.catcherstates[0]][self.actions[0]]+self.alpha * (
                    -self.features[self.catcherstates[0]][self.actions[0]] + reward + self.gamma * maxval)
            else:
            ...
            for i in reversed(range(9)):
                ...
                if nextx in range(self.getmaxx()) and nexty in range(self.getmaxy()):
                    if i in self.features[address]:
                        if (self.tolerances[tolerance][i] == 1) and (bestval == 0 or (bestval == 1 and maxval<self.features[address][i])):
                            maxval = self.features[address][i]
                            maxi = i
                            bestval = 1
                        elif maxval<self.features[address][i] and self.tolerances[tolerance][i] == 0 and bestval == 0:
